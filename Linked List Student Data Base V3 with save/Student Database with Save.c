...
Plus, it has a persistence mechanism.*/
...
void sort(list*,int); //sorts linked list
...
void load(list*); //loads files from storage.dat into linked list
void save(list); //saves the linked storage.dat
...
void sort(list *l, int mode){
    ...
    while(rep<3){
        ...
        rep++; //repeats thrice to ensure sort
    }
...
void load(list* l){ //loads files from storage.dat into linked list
    FILE *fp;
    int step=0;
    char temp[50],ch;
    student input;

    fp = fopen("storage.dat", "a"); //creates the save file if it doesn't exist, else it will simply open and close
    if (fp == NULL)
        printf("Couldn't access/create storage.dat.\n");

    fclose(fp);

    fp = fopen("storage.dat", "r"); //creates the save file if it doesn't exist, else it will simply open and close
    if (fp == NULL)
        printf("Couldn't access storage.dat.\n");

    while (fgets(temp, 50, fp) != NULL){ //parses file until NULL is reached
        temp[strlen(temp)-1]='\0'; //changes the '\n' at the end into the NULL character for termination
        if(step%8==0) //steps indicate what data is being copied
            strcpy(input.number,temp);
        else if(step%8==1)
            strcpy(input.fname,temp);
        else if(step%8==2)
            strcpy(input.mname,temp);
        else if(step%8==3)
            strcpy(input.lname,temp);
        else if(step%8==4)
            strcpy(input.program,temp);
        else if(step%8==5)
            input.yr=atoi(temp);
        else if(step%8==6)
            input.gender=temp[0];
    ...
    fclose(fp); //closes document

    return;
}

void save(list l){ //saves the linked storage.dat
    FILE *fp;
    student cast;
    int i;

    fp = fopen("storage.dat", "w"); //overwrites storage.dat for saving
    if (fp == NULL)
        printf("Couldn't access storage.dat.\n");

    for(i=1;i<l.size+1;i++){ //parses file until NULL is reached
        cast=getItem(l,i); //sets a cast for file input
        fprintf(fp,"%s\n",cast.number);
        fprintf(fp,"%s\n",cast.fname);
        fprintf(fp,"%s\n",cast.mname);
        fprintf(fp,"%s\n",cast.lname);
        ...
        fprintf(fp,"%d\n",cast.yr);
        fprintf(fp,"%c\n",cast.gender);
        fprintf(fp,"\n"); //writes a new line to separate multiple students
    }

    fclose(fp); //close document
