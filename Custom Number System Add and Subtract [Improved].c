...
int main(){
    ...
    do{ //allows you to go back to input
        do{ //creates a loop until proper inputs have been passed
            printf("\n\tNumber Calculation\n\n");

            if(proceed<2) //declares the message when input is wrong
            ...
            scanf("%d",&base);
            fflush(stdin);
            proceed=check(num1,base)+check(num2,base);

            if (proceed<2) //cleans the screen
                system("cls");
        ...
        do{ //loop for the actual operations until exit
            ...
            case '1': //does addition
                ...
                break;
            case '2':
                subtract(num1,num2,base); //does subtraction
                break;
            case '3':
                printf("\n\t\tProgram will now exit."); //exits the program
                break;
            case '4': //does fourth action which is to repeat input
                break;
            default:
                printf("\n\t\tInput is not an action."); //if someone decides to not follow instructions
                break;
            }
...
int check(char str[],int base){ //checks for the validity of input
    int end = strlen(str), i;

    for(i=0;i<end;i++){ //checks the input
    ...
    if (base<10){ //check if input is within the base [if base is less than '10' or 'A']
        for(i=0;i<end;i++){ //checks input [if base is less than '10' or 'A']
            if (str[i]>=(base+48)) //checks if str[i] is a legit remainder of %base
                return 0;
        }
    }
    else{ //check if input is within the base [if base is or more than '10' or 'A']
        for(i=0;i<end;i++){ //checks input [if base is or more than '10' or 'A']
            if (str[i]>=(base+55)) //checks if str[i] is a legit remainder of %base
                return 0;
        }
    }
...
void clean(char str[]){
    ...
    for(;i>0;i--){ //keeps on iterating until the very first digit
        if (str[i]!='0') //if str[i] is not 0, there is no need to clean
            return;
        str[i]='\0'; //cleans that element if it is 0
    }
