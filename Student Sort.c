...
void courseDis(int); //displays the course taken
void displayAll(student[]); //displays all existing students in record sheet
void search(student[]);
void sortSt(student[]);
void sortLast(student[]);
...
int main(){
    ...
    do{
        ...
        switch(sel){
        case '1': //does the enrolling //P.S. Sir, I can't pass total as a pointer, for some reason it resets back to zero once it gets into main, I can't use it on case 2 as well
            if(total<5){
...
void displayAll(student sheet[]){ //displays all existing students in record sheet
    int i;
...
void search(student sheet[]){ //searches for all students that contain that last name
    int valid=1,confirm, i; //initializes valid to 1
    char des,lastName[200];
    ...
    do{
        do{
            ...
            capitalize(lastName); //capitalize input)

            valid=alphaCheck(lastName); //checks validity of input
        ...
        for(i=0;i<5;i++){ //checks for each student in the class record
            if(strcmp(sheet[i].lastName,lastName)==0&&sheet[i].stNo[0]!='\0'){ //checks if input and last name in the record match as well as if that person is still 'in' class
                display(sheet[i]);
...
void sortSt(student sheet[]){ //sorts record by student number
    int j,i,mini;
    student tempSt;

    printf("\n\tSorting Students by Student Number...");

    for(i=0;i<5;i++){ //sorts through elements
        mini = i; //defaults the first subset as the minimum

        for(j=i+1;j<5;j++){ //compares the other elements with the elements in sub [i]
            if (strcmp(sheet[j].stNo,sheet[mini].stNo)==-1)
                mini = j;
        }

        tempSt=sheet[i]; //puts the student in sheet[i] {current} in temporary place
        sheet[i]=sheet[mini]; //puts lowest valued student no. in student[i]
        sheet[mini]=tempSt; //puts the student in temp onto the last place of the lowest valued student no.
    }
...
void sortLast(student sheet[]){ //sorts record by student number
    int j,i,mini;
    student tempSt;
    ...
    for(i=0;i<5;i++){ //sorts through elements
        ...
        for(j=i+1;j<5;j++){ //compares the other elements with the elements in sub [i]
            if (strcmp(sheet[j].lastName,sheet[mini].lastName)==-1)
                mini = j;
            else if(strcmp(sheet[j].lastName,sheet[mini].lastName)==0){
                if(strcmp(sheet[j].firName,sheet[mini].firName)==-1)
                    mini = j;
                else if(strcmp(sheet[j].firName,sheet[mini].firName)==0){
                    if(strcmp(sheet[j].midName,sheet[mini].midName)==-1)
                        mini = j;
                }
            }
        }
        ...
        tempSt=sheet[i]; //puts the student in sheet[i] {current} in temporary place
        sheet[i]=sheet[mini]; //puts the last name that should come first in student[i]
        sheet[mini]=tempSt; //puts the student in temp onto the last place of the last name that came first
    }
